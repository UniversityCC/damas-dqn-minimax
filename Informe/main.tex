...
\begin{abstract}
Este trabajo presenta la implementación de un entorno de Damas de 8$\times$8, un agente basado en Deep Q-Network (DQN) entrenable mediante auto-juego con currículo contra Minimax, y un agente Minimax con poda alfa-beta como línea base. El sistema incluye motor de reglas, codificación de estados, máscara de acciones legales, buffer de experiencia, red objetivo, Double DQN, modelado de recompensas y actualización suave de la red objetivo. Se realizaron cuatro experimentos reproducibles: curva de aprendizaje, barrido de hiperparámetros, evaluación preliminar del DQN contra Minimax(d=3) y torneo round-robin con clasificación ELO entre el DQN y cuatro variantes de Minimax (profundidades 3--6). El DQN entrenado con 4\,000 episodios usando Double DQN, modelado de recompensas, actualización suave (Polyak) y currículo contra Minimax(d=3) alcanza el tercer lugar en el torneo ELO con 1\,518.4 puntos, superando a Minimax(d=4) (1\,466.5). Es competitivo frente a Minimax(d=3) y (d=4); el aparente buen resultado frente a (d=6) en el torneo determinista es un artefacto que la evaluación estocástica corrige, revelando un techo estructural a profundidad alta (d$\geq$5). Todos los experimentos son reproducibles mediante los scripts y notebooks del repositorio.
\end{abstract}
...
El patrón del torneo determinista —no perder frente a (d=3), (d=4) ni (d=6), pero perder todo contra (d=5)— no es intuitivo y, de hecho, resulta engañoso. Como se mostró en la subsección de validación robusta, el aparente invicto frente a (d=6) es un \emph{artefacto} de la evaluación determinista: cada confrontación produce solo dos trayectorias (una por color), por lo que el resultado depende de líneas concretas y no de una superioridad estadística. La evaluación estocástica corrige este efecto y revela el cuadro real: el DQN domina a baja profundidad (d=3, d=4) pero no obtiene victorias contra (d=5) ni (d=6), es decir, existe un \emph{techo estructural} a profundidad alta. La ventaja del currículo —usar Minimax(d=3) como oponente— explica la solidez contra baja profundidad, no un supuesto dominio sobre (d=6).
...
Los experimentos confirman tres resultados principales. Primero, la infraestructura de evaluación es correcta: el módulo de torneo captura diferencias de fuerza entre agentes y la clasificación ELO refleja el rendimiento real de cada agente. Segundo, el DQN entrenado con 4\,000 episodios y las mejoras de la Fase 2 supera a Minimax(d=4) y alcanza el tercer lugar con ELO 1\,518.4; la evaluación estocástica confirma su superioridad robusta frente a (d=3) y (d=4), y un techo estructural frente a (d=5) y (d=6). Tercero, la comparación entre entrenamientos muestra que las mejoras algorítmicas (Double DQN, modelado de recompensas, actualización suave, currículo) tienen mayor impacto que simplemente aumentar el número de episodios: el modelo de 3\,000 episodios con Fase 1 ya superó al de 14\,000 episodios sin estas mejoras.
